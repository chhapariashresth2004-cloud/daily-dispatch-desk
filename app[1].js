...
async function submitReview() {
  ...
  if (totals.totalPackedCases !== Number(updatedJob.orderCaseCount || 0) && !hasAdminMismatchOverride(updatedJob)) {
    highlightMissingField("Packed cases do not match bill cases. Ask admin to override.");
    return toast("Packed cases do not match bill cases. Ask admin to override.", "error");
  }
...
function renderReviewerDetail() {
  ...
  els.reviewerBillItems.querySelector("[data-open-bill-items]")?.addEventListener("click", () => openBillItemsDialog(job.id));
...
function renderBillItemsSummary(job) {
  ...
  return `
    <div class="bill-items-compact">
      <div>
        <strong>${count ? `${count} bill item${count === 1 ? "" : "s"}` : "Bill items not captured"}</strong>
        <span>${count ? "Hidden here to keep review clean." : "Open the bill PDF if item extraction is empty."}</span>
      </div>
      <button type="button" class="secondary-button" data-open-bill-items>Open bill items</button>
    </div>
  `;
}
...
function renderBillItemsDialogContent() {
  ...
  els.billItemsDialogContent.innerHTML = `
    <table class="mini-table bill-items-table">
      <thead><tr><th>Item</th><th>Qty</th></tr></thead>
      <tbody>
        ${filtered.map((item) => `
          <tr>
            <td>${escapeHtml(item.name || item.productName || "Item")}</td>
            <td>${escapeHtml(item.quantity ?? item.qty ?? "")}</td>
          </tr>
        `).join("")}
      </tbody>
    </table>
    ${!filtered.length ? `<p class="muted-copy">No item matched your search.</p>` : ""}
  `;
}
